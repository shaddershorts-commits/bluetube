...
(function () {
  ...
  var S = { aberto: false, carregando: false, dados: null, aba: 'amigos', estados: {} };
  ...
  async function agir(act, nome, alvoEl) {
    ...
    repintar(nome, estadoDepois(act, d));
    if (S.aberto) await carregar();
    else atualizarBadge();
  }

  // Estado local imediato (o backend é a verdade; isso só evita o piscar)
  function estadoDepois(act, d) {
    var base = { estado: d.estado || 'nenhum', pode_pedir: false, bloqueado: false, espera_ate: null, dias_espera: 0 };
    if (act === 'recusar' || act === 'cancelar' || act === 'desfazer') base.pode_pedir = false;
    return base;
  }
  ...
  async function carregar() {
    ...
    S.carregando = false;
    if (!r.ok) { S.dados = null; render(); return; }
    S.dados = r.d || {};
  ...
  function render() {
    ...
    if (!box) return;
    if (!S.dados) { box.innerHTML = '<div class="cba-vazio">Carregando…</div>'; return; }
    ['amigos', 'recebidos', 'enviados'].forEach(function (k) {
