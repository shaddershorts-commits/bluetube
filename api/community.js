...
module.exports = async function handler(req, res) {
  ...
  const linhasComigo = async (uids) => {
    ...
    // ⚠️ MEIA RESPOSTA NÃO VIRA VERDADE. Aqui havia
    // `[...(ra.ok ? await ra.json() : []), ...(rb.ok ? ...)]`, que misturava
    // sucesso com falha em silêncio: bastava `rb` engasgar pra que TODOS os
    // pares em que eu sou o lado B sumissem do mapa — e o feed então pintava
    // "+ Amigo" no cabeçalho de quem já é meu amigo. Devolver null é dizer
    // "não sei", que é diferente de "não tem"; quem chama apaga a pílula.
    if (!ra.ok || !rb.ok) return null;
    const rows = [...(await ra.json()), ...(await rb.json())];
    const map = {};
  ...
  try {
    ...
    if (action === 'feed') {
      ...
      const fmap = await linhasComigo(uids).catch(() => null);
      const out = posts.map((p) => ({
        ...
        amizade: (p.user_id === userId || !fmap) ? null : AM.estadoParaMim(fmap[String(p.user_id).toLowerCase()] || null, meId),
      }));
    ...
    if (action === 'comments') {
      ...
      return res.status(200).json({
        comments: comments.map((c) => ({
          ...
          amizade: (c.user_id === userId || !fmap) ? null : AM.estadoParaMim(fmap[String(c.user_id).toLowerCase()] || null, meId),
        })),
        is_moderator: isMod,
      });
    }
    ...
    if (action === 'amizades') {
      ...
      // mais votado da revisão (6 de 6 revisores acharam sozinhos). Ele
      // transformava QUALQUER falha — tabela ausente, 401, timeout, 5xx — numa
      // lista vazia, e o handler devolvia 200. Falha de leitura e "não tenho
      // amigos" saíam byte a byte IGUAIS, então a tela dizia com toda a
      // confiança "🤝 Você ainda não tem amigos por aqui" e zerava o contador.
      //
      // Não é hipótese: medido em 11/08, community_friendships responde 404
      // PGRST205 porque sql/comunidade_amizades.sql nunca foi rodado. Sem esta
      // guarda, subir as amizades hoje entregaria um painel vazio e convincente
      // pra todo assinante — e ninguém teria como saber que faltava o banco.
      if (!r.ok) {
        const detalhe = await r.text().catch(() => '');
        console.error('[amizades] leitura falhou', r.status, detalhe.slice(0, 200));
        return res.status(503).json({
          error: 'Não deu pra carregar seus amigos agora. Tente de novo em instantes.',
          degradado: true,
          // Some pro usuário, aparece pro dono: diz o que RODAR, em vez de
          // deixar a pessoa caçando (foi a lição da fila da BlueScore).
          detalhe: /does not exist|PGRST205/i.test(detalhe)
            ? 'A tabela community_friendships ainda não existe — rode sql/comunidade_amizades.sql no Supabase.'
            : undefined,
        });
      }
