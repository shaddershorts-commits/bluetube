...
<div style="font-family:Arial,sans-serif;background:#0a1628;color:#e8f4ff;padding:32px 24px;border-radius:16px;max-width:560px;margin:0 auto">
  ...
  if (req.method === 'GET' && action === 'realtime_pulse') {
    try {
      ...
      const [countRes, onlineRes, cancelRes, latestRes, payCountRes, latestPayRes] = await Promise.all([
        ...
        // Pagantes (full+master) — pra celebrar novas assinaturas no painel
        fetch(`${SUPABASE_URL}/rest/v1/subscribers?plan=in.(full,master)&select=id`, { headers: { ...headers, 'Prefer': 'count=exact' } }),
        fetch(`${SUPABASE_URL}/rest/v1/subscribers?plan=in.(full,master)&select=email,plan&order=updated_at.desc&limit=1`, { headers }),
      ]);
      const totalHeader = countRes.headers?.get('content-range');
      const total = totalHeader ? parseInt(totalHeader.split('/')[1]) || 0 : 0;
      const payHeader = payCountRes.headers?.get('content-range');
      const paying = payHeader ? parseInt(payHeader.split('/')[1]) || 0 : 0;
      ...
      const latestPay = latestPayRes.ok ? (await latestPayRes.json())[0] : null;
      return res.status(200).json({
        total,
        paying,
        online_now: online,
        latest_email: latest?.email || null,
        latest_paying_email: latestPay?.email || null,
        latest_paying_plan: latestPay?.plan || null,
