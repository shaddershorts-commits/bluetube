...
// Plano REAL do dono do token, conferido no banco — não no que a tela diz.
// Mesma conta de vencimento que o resto do site usa: `is_manual` vale pra
// sempre, senão a data manda. Qualquer tropeço devolve 'free', porque a falha
// de um portão tem que fechar, não abrir.
async function planoDoToken(token, SU) {
  if (!token) return 'free';
  try {
    const AK = process.env.SUPABASE_ANON_KEY;
    const SK = process.env.SUPABASE_SERVICE_KEY;
    const ur = await fetch(`${SU}/auth/v1/user`, { headers: { apikey: AK, Authorization: 'Bearer ' + token } });
    if (!ur.ok) return 'free';
    const u = await ur.json();
    if (!u || !u.email) return 'free';
    const sr = await fetch(
      `${SU}/rest/v1/subscribers?email=eq.${encodeURIComponent(u.email)}&select=plan,plan_expires_at,is_manual&limit=1`,
      { headers: { apikey: SK, Authorization: 'Bearer ' + SK } },
    );
    if (!sr.ok) return 'free';
...
module.exports = async function handler(req, res) {
  ...
  try {
    // ── PORTÃO DE MASTER (15/08/2026) ────────────────────────────────────
    // "Vídeos Longos" virou opção do dropdown da Virais, EXCLUSIVA do Master
    // (pedido do dono). Esconder a opção na tela não protege nada: sem esta
...
async function coletar(req, res, { SU, h }) {
  ...
  // busca rotacionava os 4 países fixos mesmo com `paises=US` no filtro: 3 de
  // cada 4 buscas voltavam com canal que ia ser descartado depois — cota
  // gasta em vídeo que nunca entraria. Medido em 15/08, quando o dono pediu
  // foco em canal de língua inglesa.
  ...
  for (let i = 0; i < termos.length; i++) {
    ...
    // Rotaciona os mercados pedidos entre as buscas da rodada: cada termo sai
    // olhando um país, e ao longo da rodada todos são cobertos.
    const pais = mercados[i % mercados.length] || PAISES[i % PAISES.length];
    try {
