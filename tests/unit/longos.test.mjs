...
// 15/08: a entrada deixou de ser item de barra e virou opção do dropdown de
// nichos da Virais, exclusiva do Master (pedido do dono). O que precisa ser
// travado agora é o CAMINHO NOVO — e que ele não seja só cosmético.
const VIRAIS = readFileSync(new URL('../../public/virais.html', import.meta.url), 'utf8');

test('a entrada é a opção do dropdown da Virais, abaixo do Nicho Secreto', () => {
  const i0 = VIRAIS.indexOf("value=\"__secreto\"");
  const i1 = VIRAIS.indexOf("value=\"__longos\"");
  assert.ok(i1 > 0, 'a opção Vídeos Longos sumiu do dropdown');
  assert.ok(i1 > i0, 'ela tem que vir DEPOIS do Nicho Secreto, como foi pedido');
  assert.match(VIRAIS, /Vídeos Longos/);
});

test('o portão de Master é do SERVIDOR, não só da tela', () => {
  // Esconder a opção não protege nada: sem checagem no endpoint, qualquer um
  // abre /api/longos?action=listar no navegador e lê o acervo inteiro.
  assert.match(API, /action === 'listar' \|\| action === 'canais'/);
  assert.match(API, /plano !== 'master'/);
  assert.match(API, /master_only/);
  // e a tela também confere, pra o Full não bater em erro seco
...
test('o padrão segue os 4 países pedidos, e eles entram na BUSCA também', () => {
  assert.deepEqual(I.PAISES, ['US', 'ES', 'BR', 'DE'], 'o padrão não muda sem pedido do dono');
  // regionCode sozinho devolveria conteúdo em inglês mirando a Alemanha.
  for (const [pais, idioma] of [['US', 'en'], ['ES', 'es'], ['BR', 'pt'], ['DE', 'de']]) {
    assert.equal(I.IDIOMA_DO_PAIS[pais], idioma);
  }
  const busca = API.slice(API.indexOf('// ── 1) BUSCA'), API.indexOf('stat.candidatos'));
...
test('a BUSCA mira os mesmos mercados que o filtro aceita', () => {
  // Bug medido em 15/08: com `paises=US` o filtro virava US-only mas a busca
  // continuava rotacionando os 4 fixos — 3 de cada 4 buscas voltavam com canal
  // que seria descartado depois. `search` custa 100 unidades; era cota jogada
  // fora em vídeo que nunca entraria.
  const busca = API.slice(API.indexOf('// ── 1) BUSCA'), API.indexOf('stat.candidatos'));
  assert.match(busca, /mercados\[i % mercados\.length\]/,
    'a busca tem que seguir `paises=`, senão o foco por mercado só filtra depois de gastar');
  assert.match(API, /const mercados = String\(req\.query\.paises \|\| PAISES\.join/,
    'mercados da busca e países do filtro saem do MESMO parâmetro');
});

test('os mercados de língua inglesa têm idioma definido', () => {
  // Só os EUA deixava metade do mercado anglófono de fora (pedido de foco em
  // inglês, 15/08). Eles não entram no padrão — chegam por `paises=`.
  for (const p of ['GB', 'CA', 'AU']) {
    assert.equal(I.IDIOMA_DO_PAIS[p], 'en', `${p} sem idioma cairia no fallback sem querer`);
    assert.equal(I.PAISES.includes(p), false, `${p} não pode entrar no padrão sem o dono pedir`);
  }
});
